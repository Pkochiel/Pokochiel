...
import { IMAGE_MEMORY, scoreImageMemory } from './image-memory'

const WORDS = Array.from({ length: 40 }, (_, i) => `語${i + 1}`)

describe('IMAGE_MEMORY の設定', () => {
  it('40語・2セット', () => {
    expect(IMAGE_MEMORY.wordCount).toBe(40)
    expect(IMAGE_MEMORY.sets).toBe(2)
  })

  it('初級は2分', () => {
    expect(IMAGE_MEMORY.timeLimits[0]).toBe(120_000)
  })

  it('級が上がるごとに短くなる', () => {
    const limits = IMAGE_MEMORY.timeLimits
  ...
  it('上がる基準が戻る基準より高い', () => {
    expect(IMAGE_MEMORY.advanceRecalled).toBeGreaterThan(IMAGE_MEMORY.fallbackRecalled)
  })
})

describe('scoreImageMemory', () => {
  it('再生できた語を数える', () => {
    const result = scoreImageMemory(WORDS, [{ index: 0, recalled: WORDS.slice(0, 12) }])
    expect(result.sets[0]?.recalled).toBe(12)
  })

  it('各セットのスコアを並びとして残す', () => {
    const result = scoreImageMemory(WORDS, [
  ...
  it('同じ語を並べても1回として数える', () => {
    const first = WORDS[0]!
    const result = scoreImageMemory(WORDS, [{ index: 0, recalled: [first, first, first, first] }])
  ...
  it('空行は数えない', () => {
    const result = scoreImageMemory(WORDS, [{ index: 0, recalled: ['', '   ', '　', WORDS[0]!] }])
    expect(result.sets[0]?.recalled).toBe(1)
    expect(result.sets[0]?.intruded).toBe(0)
  })

  it('全問正解なら出題数に一致する', () => {
    const result = scoreImageMemory(WORDS, [{ index: 0, recalled: WORDS }])
